...
logging.basicConfig(filename='./log/btc.log', level=logging.DEBUG)
...
def on_message(ws, message):
	...
	for event in events:
		with getcursor() as conn:
			...
			try:
				if event['type'] == 'trade':
					cursor.execute(db_libs.trade_insert_str.format('btc', 'BTC', event['makerSide'] == 'ask', event['price'], event['amount']))
				else:
					cursor.execute(db_libs.bid_ask_insert_str.format('btc', 'BTC', event['side'] == 'ask', event['price'], event['remaining']))
